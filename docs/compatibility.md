...
Status: **Shared framework and Codex adapter implemented; seven original v1 adapters remain pending.** GitHub Copilot CLI is a documentation-only candidate awaiting separate approval, and Cursor Agent CLI is a documentation-only blocked design. Public sources for the original set were checked on 2026-09-14 (America/New_York); Copilot and Cursor sources were checked on 2026-09-15. Codex CLI 0.154.0 was locally smoke-checked on 2026-09-15 without inference. Source links and detailed native mappings are in each adapter spec. Documentation can describe a newer release than the user's installed binary; detection selects only verified behavior.
...
| [GitHub Copilot CLI](../harnesses/copilot/SPEC.md) | Child-scoped routing environment; no native profile | Child environment | Exact selected model per launch | Shared |
...
| GitHub Copilot CLI | Candidate | Candidate | No | No |
...
- **GitHub Copilot CLI:** pending separate approval; pin the BYOK capability/version, clear inherited provider routing, reject model-changing arguments and keep provider credentials child-scoped.
